# UPDATE (August 15, 2016): Scoring method modified; UI customized for Windows
...
class ScoreBoard:
    ...
    def __init__(self,win):
        ...
        self.level_text = UItext(Point(0.5*self.width,0.3*self.height),
                          "Level\n%i" %self.level, 16, 'bold', self.canvas)
        self.score = 0
        self.score_text = UItext(Point(0.5*self.width,0.7*self.height),
                          "Score\n%i" %self.score, 16, 'bold', self.canvas)
...
class Preview:
    ...
    def __init__(self,win):
        ...
        self.canvas = CanvasFrame(win,self.width,self.height)
        self.canvas.setBackground('black')
        self.preview_text = UItext(Point(0.5*self.width,0.2*self.height),
                                 "Next Piece", 16, 'bold', self.canvas)
